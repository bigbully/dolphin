...
class AsyncProducer(private[this] val id: String, private[this] val conf: ClientConfig) extends Producer(id, conf) {
  ...
  val queue = new LinkedBlockingQueue[Message](conf.get(QUEUE_BUFFERING_MAX_MESSAGES).get.asInstanceOf[Int])
  val enqueueStrategy: (Message => Unit) = conf.get(PRODUCER_ENQUEUE_TIMEOUT_MS).get match {
  ...
  var registerFunction: Option[(ActorRef, TopicModel) => Unit] = None

  def startSendThread {
    sendThread.start()
  }

  var enrollActFuture:Future[ActorRef] = _
  ...
  override def publish(topic: String, cluster: String) {
    topicModel = TopicModel(topic, cluster)
    enrollActFuture onSuccess {
      case act => {
        act ! RegisterClient(this, ClientModel(id, PRODUCER), topicModel)
        enrollAct = act
      }
    }
  }
  ...
  override def send(msg: Array[Byte]) {
    enqueueStrategy(Message(msg, topicModel.name.getBytes(UTF_8), None, false))
  }
}
